...
func RegisterRoutes(router *mux.Router) {
	...
	router.HandleFunc("/webhook_sales", handleBlingWebhookSales).Methods(http.MethodPost)
}

func handleBlingWebhookSales(w http.ResponseWriter, r *http.Request) {
	...
	log.Println("Recebendo webhook de atualização de vendas da Bling...")
	...
	for _, estoqueWrapper := range response.Retorno.Estoques {
		...
		if err := updateProductInDatabase(estoque); err != nil {
			log.Printf("Falha ao atualizar a venda  na base de dados: %v", err)
			http.Error(w, "Failed to update sales", http.StatusInternalServerError)
			return
		}
	}
